...
SELECT category, COUNT(category) AS number_of_apps
FROM analytics
GROUP BY category;
...
SELECT app_name
FROM analytics
WHERE rating < 3.0 AND reviews > 10000;
--Find the top 10 most-reviewed apps that cost between 10 cents and a dollar.--
SELECT app_name
FROM analytics
WHERE price > 0.1 AND price < 1.0
ORDER BY reviews DESC
LIMIT 10;
...
LIMIT 1;
--11.Using subquery Find the most out of date app. Hint: You don’t need to do it this way, but it’s possible to do with a subquery: http://www.postgresqltutorial.com/postgresql-max-function/--
SELECT app_name, last_updated
FROM analytics
...
SELECT SUM(reviews) AS all_the_reviews
FROM analytics;
...
SELECT app_name
FROM analytics
WHERE array_length(genres, 1) > 1 AND genres IS NOT NULL;
